...
const NavLink = ({ title, href, onClick, isMobile }) => {
    return (
      <a
        href = {href}
        className={`group relative px-4 ${isMobile ? "px-3" : "py-2"} 
        rounded-xl text-gray-700 font-medium transition-color duration-200
        cursor-pointer`}
        onClick={onClick}
      >
        ...
        {!isMobile && (
        <div className="absolute inset-0 bg-red-50 rounded-xl scale-90 
